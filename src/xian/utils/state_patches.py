...
def compile_contract_from_source(s: dict):
    ...
    # Extract contract name from the key (e.g., "con_token.__code__" -> "con_token")
    contract_name = s["key"].split(".")[0]
    
    # Create compiler with the correct module name
    compiler = ContractingCompiler(module_name=contract_name)
    
    # First transform the code to add privatization, ORM objects, etc.
    transformed_code = compiler.parse_to_code(s["value"])
    
    # Now compile the transformed code to bytecode
...
class StatePatchManager:
    ...
    def apply_patches_for_block(self, height, nanos):
        ...
        for patch in patches:
            ...
            if len(parts) > 1 and parts[1] == "__code__":
                ...
                # Try to compile the contract code, but handle errors gracefully
                try:
                    transformed_code, compiled_code = compile_contract_from_source(patch)
                    
                    # Set both the source code and compiled code
                    self.raw_driver.set(key, transformed_code)  # Set the transformed source code
                    self.raw_driver.set(f"{contract_name}.__compiled__", compiled_code)  # Set the compiled code
                    
                    # Add the compiled code to applied patches for BDS recording
                    compiled_patch = {
                ...
                # Skip the general case below since we've handled this patch specially
                continue
